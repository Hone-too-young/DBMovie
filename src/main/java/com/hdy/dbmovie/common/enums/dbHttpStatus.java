...
public enum dbHttpStatus {
    ...
    dbHttpStatus(int value, String msg) {
        this.value = value;
        this.msg = msg;
    }
    ...
    public static dbHttpStatus valueOf(int statusCode) {
        dbHttpStatus status = resolve(statusCode);
        if (status == null) {
    ...
    public static dbHttpStatus resolve(int statusCode) {
        for (dbHttpStatus status : values()) {
