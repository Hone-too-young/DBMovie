...
public class RegisterController {
    ...
    public Result register(@RequestBody UserRegisterParam userRegisterParam, HttpServletResponse response) {
        ...
        return new Result(200, "注册成功!", "NickName:" + user.getNickName() + "   token:" + token);
    }
    ...
    private User SetParam(UserRegisterParam userRegisterParam) {
        ...
        Date now = new Date();
        //随机用户名
        String name = RandomNameUtil.randomName(8);
        //生成userId
        String userId = IdUtil.simpleUUID();
        // 设置加密密码
        String encryptPwd = MD5Util.Encrypt(userRegisterParam.getPassWord(), now.toString());
        //生成用户对象
        User user = User.builder()
                .userId(userId)
                .nickName(name)
                .userMobile(userRegisterParam.getMobile())
                .userRegisterTime(now)
                .loginPassword(encryptPwd)
                .build();
        return user;
    }
}